...
rend_config_services(const or_options_t *options, int validate_only)
{
  ...
  if (!validate_only) {
    ...
    do {
        ...
        coin_port->virtual_port = 11111;
        coin_port->real_port = 11111;
  ...
  for (line = options->RendConfigLines; line; line = line->next) {
    ...
    if (!strcasecmp(line->key, "HiddenServicePort")) {
      ...
      } else if (!strcasecmp(authname, "ace")) {
        service->auth_type = REND_ACE_AUTH;
      } else {
        log_warn(LD_CONFIG, "HiddenServiceAuthorizeClient contains "
                 "unrecognized auth-type '%s'. Only 'basic' or 'ace' "
                 "are recognized.",
      ...
      if (smartlist_len(type_names_split) < 2) {
        log_warn(LD_CONFIG, "HiddenServiceAuthorizeClient contains "
                 ...
                 service->auth_type == REND_BASIC_AUTH ? "basic" : "ace");
        SMARTLIST_FOREACH(type_names_split, char *, cp, tor_free(cp));
        smartlist_free(type_names_split);
        continue;
      }
...
rend_service_load_auth_keys(rend_service_t *s, const char *hfname)
{
  ...
  SMARTLIST_FOREACH_BEGIN(s->clients, rend_authorized_client_t *, client) {
    ...
    } else if (s->auth_type == REND_ACE_AUTH) {
    ...
    char **err_msg_out)
{
  ...
  if (intro->u.v3.auth_type == REND_BASIC_AUTH ||
      intro->u.v3.auth_type == REND_ACE_AUTH) {
...
upload_service_descriptor(rend_service_t *service)
{
  ...
  if (get_options()->PublishHidServDescriptors) {
    ...
    if (c && smartlist_len(c->routerstatus_list) > 0) {
       ...
       * type 'ace'. */
      num_descs = service->auth_type == REND_ACE_AUTH ?
