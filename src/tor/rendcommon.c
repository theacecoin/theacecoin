...
/** Encrypt the encoded introduction points in <b>encoded</b> using
 * authorization type 'ace' with <b>descriptor_cookie</b> of length
...
static int
rend_encrypt_v2_intro_points_ace(char **encrypted_out,
                                     size_t *encrypted_len_out,
                                     const char *encoded,
                           ...
                           smartlist_t *client_cookies)
{
  ...
  service_key = (auth_type == REND_ACE_AUTH) ? client_key : desc->pk;
  tor_assert(service_key);
  if (auth_type == REND_ACE_AUTH) {
  ...
  if (smartlist_len(desc->intro_nodes) > 0) {
    ...
    switch (auth_type) {
      ...
      case REND_BASIC_AUTH:
        ...
        break;
      case REND_ACE_AUTH:
        if (rend_encrypt_v2_intro_points_ace(&ipos_encrypted,
                                                 &ipos_encrypted_len, ipos,
                                                 descriptor_cookie) < 0) {
          log_warn(LD_REND, "Encrypting of introduction points did not "
                            "succeed.");
