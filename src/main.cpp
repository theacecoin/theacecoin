...
int64 nChainStartTime = 1406268803;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    int64_t nSubsidy = 0;
    if (nHeight <= 50)
        nSubsidy = 1 * COIN;    // Low reward period for fairness.
    else if (nHeight <= CUTOFF_POW_BLOCK)
        nSubsidy = 350 * COIN;  // Primary distribution up to block 2907.
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_ACE_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in AceCoin
    bool fStrictPayToScriptHash = true; // Always active in AceCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    if ((nTime >= CUTOFF_POW_TIME) && (IsProofOfWork()))
         return DoS(100, error("CheckBlock() : Proof of work (%f ACE) at t=%d on or after %d.\n",
...
bool CBlock::AcceptBlock()
{
    ...
    // Hack. We allowed stake by accident below block 1300.
    // The policy is forced to change here between 1300 and CUTOFF_POW_BLOCK.
    if (IsProofOfStake() && (nHeight > 1300) && (nHeight <= CUTOFF_POW_BLOCK))
        return DoS(100, error("AcceptBlock() : no PoS blocks allowed between 1300 and %d", CUTOFF_POW_BLOCK));
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "20140729 ABC Online- Russia hit with strongest sanctions since Cold War";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1406683759;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4207494668; // perfect nonce
        ...
        assert(block.hashMerkleRoot == uint256("0x96035c70c0fbf5a55f8d5494b22c836217a7d1af778a02185a90ffaccae6b0dd"));
...
unsigned char pchMessageStart[4] = { 0x71, 0x35, 0x23, 0x06 };
    ...
    if (strCommand == "version")
    {
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (addrman.CheckVerificationToken(addrFrom, verification_token)) {
                ...
                // HACK: For now we're accepting connections without a this because
                // the test is currently broken and we need to not fragment the network.
                printf("(TEMP HOTFIX) connection from %s verified\n", addrFrom.ToString().c_str());
                pfrom->fVerified = true;
                addrman.Good(pfrom->addr);
            }
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake && pindexBest->nHeight >= CUTOFF_POW_BLOCK)  // attempt to find a coinstake
    {
